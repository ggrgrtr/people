# from collections import deque # 2очередь
from pathlib import Path
import math
import time

# класс для сглаживания оценки FPS с помощью экспоненциального скользящего среднего
class RateMeter:
    ...
    def update_delta(self, dt):
        ...
        if self.value <= 0.0:
            ...
            # сглаживание с помощью экспоненциального скользящего среднего
            self.value = (1.0 - self.alpha) * self.value + self.alpha * instant_rate
        return self.value

# упрощение названий данных
...
# создание дерева папок с выходными данными 
def build_output_paths(output_dir, source_label):
    # создаем корневую папку для всех сессий трекинга, если ее нет
    output_dir.mkdir(parents=True, exist_ok=True)
    # метка с датой и временем для уникальности папки с результатами
    stamp = time.strftime("%Y%m%d_%H%M%S")
    # имя папки запуска
    source_name = sanitize_name(source_label)
    # корневая папка
    session_dir = output_dir / f"{source_name}_{stamp}"
    # создаем папку для сохранения видео, логов и результатов трекинга
...
# определение видео: если цифра - камера, если строка - путь к файлу 
...
# поиск по папкам файла с детекцией ёло 
def resolve_yolo_weights(base_dir):
    ...
    # если не нашелся, возвращаем дефолтное имя, чтобы загрузить его из интернета
    return "yolov8n.pt"

# проверка, что FPS в разумных пределах
...
# поиск по папкам файла с весами для reid модели
def resolve_reid_weights(base_dir, explicit_path=""):
    candidates = []
    # если явно указано, сначала проверяем этот путь, а затем варианты расположения
    if explicit_path:
    ...
    seen = set()
    for candidate in candidates:
        # для оптимизации разрешаем путь только если он абсолютный, иначе используем его как есть
...
# обрезание рамки по границам кадра, если рамка выходит за них, возвращаем обрезанную рамку
...
# вычисление IoU между двумя рамками, если одна из рамок None, возвращаем 0
...
# вычисление площади рамки, если рамка None, возвращаем 0
def bbox_area(bbox):
    return max(0, int(bbox[2])) * max(0, int(bbox[3]))

# вычисление отношения площади пересечения рамок к площади меньшей рамки, если одна из рамок None, возвращаем 0
...
# сглаживание положения рамки: если предыдущая точка None, возвращаем новую точку, иначе сглаживаем по формуле: prev + alpha * (new - prev)
...
# сглаживание рамки: если центр или размер не изменились сильно, то сглаживаем сильнее, иначе - меньше
def smooth_bbox(
    prev_bbox,
    new_bbox,
    frame_shape,
    ...
    # сглаживаем отдельно положение и размеры рамки
    ...
    # мертвая зона, при маненких колебаниях рамка почти не меняется
    ...
    # создаем новую рамку из сглаженных центра и размера, затем обрезаем ее по границам кадра
    return clip_bbox(
        bbox_from_center(smooth_center, smooth_wh[0], smooth_wh[1]),
        frame_shape,
    )

# добавление точку в маршрут дввижения человека, если она не меньше min_distance от последней добавленной точки
...
# придумываем цвет исходя из номера айди
